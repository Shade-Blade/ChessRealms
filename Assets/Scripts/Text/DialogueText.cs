    ...
    public static string ConvertSymbolTag(TagEntry tag)
    {
        ...
        switch (tag.tag)
        {
            ...
            case TagEntry.TextTag.Infinity:
                output = "\u221E";
                break;
            case TagEntry.TextTag.Square:
                output = "\u25A0";
                break;
            case TagEntry.TextTag.EmptySquare:
                output = "\u25A1";
                break;
        }
        return output;
    ...
    public static string ConvertTagEntry(TagEntry tag)
    {
        ...
        switch (tag.tag)
        {
            ...
            case TagEntry.TextTag.Const:
                ...
                break;
            case TagEntry.TextTag.OutlineColor:
                if (!tag.open)
                {
                    output = "</color></font>";
                }
                else
                {
                    ...
                    {
                        output = "<font=\"Rubik-SemiBold SDF\" material=\"Rubik-SemiBold Atlas Material OutlineColorMatch\"><color=#ffffff>";
                        break;
                    }
                    ColorNames color;
            ...
            case TagEntry.TextTag.OutlineInvert:
                ...
                else
                {
                    if (tag.args == null || tag.args.Length == 0)
                    {
                        output = "<font=\"Rubik-SemiBold SDF\" material=\"Rubik-SemiBold Atlas Material OutlineColorInvert\"><color=#ffffff>";
                        break;
                    }
                    ColorNames color;
                    if (Enum.TryParse(tag.args[0], out color))
                    ...
                    output = "<font=\"Rubik-SemiBold SDF\" material=\"Rubik-SemiBold Atlas Material OutlineColorInvert\"><color=" + tag.args[0] + ">";
                }
                break;
            /*
...
public class FormattedString
{
    ...
    */
    ...
    public static string ReplaceTextFileTags(string s)
    {
        ...
        for (int i = tags.Length - 1; i >= 0; i--)
        {
            TagEntry.TextTag[] tagsToCheck = new TagEntry.TextTag[] { }; // new TagEntry.TextTag[] { TagEntry.TextTag.MenuText, TagEntry.TextTag.CommonText, TagEntry.TextTag.SystemText, TagEntry.TextTag.ItemText, TagEntry.TextTag.KeyItemText, TagEntry.TextTag.LocalText };
            bool success = false;
            for (int j = 0; j < tagsToCheck.Length; j++)
            {
                if (tags[i].tag == tagsToCheck[j])
